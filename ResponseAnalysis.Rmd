...
TarpTime<-TarpTime%>%filter(!is.na(Val_sqf))
...
SummaryDat<-TarpTime%>%
          st_join(matchIndicator, by=FootprintID, left=TRUE)%>% 
            mutate(status = (ifelse(is.na(matched) & Program == 1,"Unmatched_Treated", 
                             ifelse(is.na(matched) & Program == 0,"Unmatched_Untreated", 
                             ifelse(matched == 1 & Program == 1,"Matched_Treated", 
                             ifelse(matched == 1 & Program == 0,"Matched_Unreated", NA))))))
