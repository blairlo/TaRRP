...
Install_Predicted<-st_read("Building Data/Install_Predicted.shp")%>%
                              ...
                              select(-TarpP6)%>%
                              mutate(Program = 1)
Install_Predicted<-rbind(NInstall_Predicted,Install_Predicted)


Install_Predicted$FootprintArea <- st_area(Install_Predicted)
Install_Predicted$FootprintArea<-drop_units(Install_Predicted$FootprintArea)
...
#spatial joins
ProgramTarpTimeUnfilter <- Install_Predicted%>%
...
ROEDist<-DistToNear(LayerX = ProgramTarpTimeUnfilter,
                 LayerY = ROELocation,
...
InterstateDis<-DistToNear(LayerX = ProgramTarpTimeUnfilter,
                 LayerY = osm_line,
                 VarY = osm_line$highway,
                 FeatureY = "trunk"
                )

HighwayDist<-DistToNear(LayerX = ProgramTarpTimeUnfilter,
                 LayerY = osm_line,
                 VarY = osm_line$highway,
                 FeatureY = "primary"
                )
...
ProgramTarpTimeUnfilter <- ProgramTarpTimeUnfilter%>%
...
#make zipcodes a factor
ProgramTarpTimeUnfilter$DSITEZIP<-as.factor(ProgramTarpTimeUnfilter$DSITEZIP)
...
```{r}
#make new vars
ProgramTarpTime <- ProgramTarpTimeUnfilter%>%
...
ProgramTarpTime$BuildingType<-relevel(ProgramTarpTime$BuildingType, "SINGLE FAMILY")

#Select only tarped properties and remove outliers
ProgramTarpTime<-ProgramTarpTime%>%
...
ProgramTarpTime <- ProgramTarpTime%>%
                    mutate(CovrPrd2 = ifelse(CovrPrd == 10 , 8, CovrPrd))

#Continue with Program, NonProgram, or Both?
run = "Program"
if (run=="Program") {
  test<-filter(ProgramTarpTime, Program==1)
  
  if (run=="NonProgram") {
    test<-filter(ProgramTarpTime, Program==0)
    
    if (run=="Both") {
      test<-ProgramTarpTime
  }}}
...
plot1<-ggplot(summarySE(ProgramTarpTime, measurevar="CovrPrd", groupvars=c("HomeSizeQaurtiles")),
...
#Intercept Only
Reg1a <- glm(((CovrPrd/2)-1)~1
             , family="poisson"
             , data = ProgramTarpTime)

#Base Reg
Reg2a <- glm(((CovrPrd/2)-1)~
           log(FootprintArea)
           +log(VAPBLDG)
           , family="poisson"
           , data = ProgramTarpTime)
...
table4<-stargazer(Reg1c, Reg2c, Reg3c,Reg4c, Reg5c,
