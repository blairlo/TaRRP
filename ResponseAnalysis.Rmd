...
TarpTime <- TarpTimeUnfilter%>%
                          mutate(Val_sqf = VAPTOTAL/S1AREATTL,
...
stat1 <- ddply(TarpTime, "Program", summarise, Val.stat1=mean(Val_sqf))
stat1

plot4 <- ggplot(TarpTime, aes(x=Val_sqf, fill=as.factor(Program))) +
...
matchProgram_df$CovrPrd<-as.factor(matchProgram_df$CovrPrd)
...
summary(Reg4g)
summary(Reg5g)
test<-ggpredict(Reg5g, c("FootprintArea [all]", "Program"))
plot(test)
