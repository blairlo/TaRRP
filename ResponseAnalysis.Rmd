...
TarpTime <- TarpTime%>%
                          mutate(AssessValQaurtiles = 
                            ifelse(CutQ(VAPBLDG) == "Q1","Q1AssessVal",
                            ifelse(CutQ(VAPBLDG) == "Q2","Q2AssessVal",
                            ifelse(CutQ(VAPBLDG) == "Q3","Q3AssessVal",
                            ifelse(CutQ(VAPBLDG) == "Q4","Q4AssessVal",NA)))))
...
plot1<-ggplot(summarySE(ProgramTarpTime, measurevar="CovrPrd2", groupvars=c("HomeSizeQaurtiles")),
              aes(x=HomeSizeQaurtiles, y=CovrPrd2, group=1))+
                geom_errorbar(width=.1, aes(ymin=CovrPrd2-ci, ymax=CovrPrd2+ci))+
                 geom_point(shape=21, size=3, fill="white")
plot1



Plot2 <- ggplot(summarySE(ProgramTarpTime, measurevar="CovrPrd2", groupvars=c("HomeSizeQaurtiles","HomeValLS")),
       aes(x=HomeSizeQaurtiles, y=CovrPrd2, colour=HomeValLS, group=HomeValLS)) +
  
    geom_errorbar(aes(ymin=CovrPrd2-se, ymax=CovrPrd2+se), colour="black", width=.1) +
    geom_line() +
...
Plot3<- ggplot(ProgramTarpTime, aes(x = CovrPrd2, y = HomeSizeQaurtiles  )) +
        geom_boxplot(size = .5) +
        geom_jitter(alpha = .1) +
...
```{r}
#Intercept Only
Reg1 <- lm(CovrPrd2~1, data = ProgramTarpTime)
...
```{r}
#Intercept Only
Reg1b <- clm(as.factor((CovrPrd2/2)-1)~ 1
           , data = ProgramTarpTime)

#Base Reg
Reg2b <- clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)
           +log(VAPBLDG)
           , data = ProgramTarpTime)
...
#Property Level Controls
Reg3b <- clm(as.factor((CovrPrd2/2)-1)~
           log(FootprintArea)
           +log(VAPBLDG)
           +log(X1mSWmphBnd)
           +age+I(age^2)
...
#Block Group socioeconomic controls
Reg4b <- clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)
           +log(VAPBLDG)
           +log(X1mSWmphBnd)
           +age+I(age^2)
...
# Zip Code FE
Reg5b <-  clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)
           +log(VAPBLDG)
           +log(X1mSWmphBnd)
           +age+I(age^2)
...
ProgramTarpTime$LVAPBLDG<-log(ProgramTarpTime$VAPBLDG)
...
```{r}
#Intercept Only
Reg1c <- clm(as.factor((CovrPrd2/2)-1)~ 1
           , data = ProgramTarpTime)

#Base Reg
Reg2c <- clm(as.factor((CovrPrd2/2)-1)~
           log(FootprintArea)
           +log(VAPBLDG)
           , data = ProgramTarpTime)
...
#Property Level Controls
Reg3c <- clm(as.factor((CovrPrd2/2)-1)~
          log(FootprintArea)
           +log(VAPBLDG)
           +age+I(age^2)
           +BuildingType
...
#Block Group socioeconomic controls
Reg4c <- clm(as.factor((CovrPrd2/2)-1)~
           log(FootprintArea)
           +log(VAPBLDG)
           +age+I(age^2)
           +BuildingType
...
# Zip Code FE
Reg5c <-  clm(as.factor((CovrPrd2/2)-1)~
           log(FootprintArea)
           +log(VAPBLDG)
           +age+I(age^2)
           +BuildingType
...
#Matched Set     
matchSize <- matchit(SizeTreatLarge ~ VAPBLDG 
                                    +X1mSWmphBnd
                                    +age
                                    +BuildingType
                                    +DistROE
...
matchSize_Control<- mean(matchSize_Control$CovrPrd2)

matchSize_Treat<- matchSize_df%>%
                      filter(SizeTreatLarge==1)

matchSize_Treat<- mean(matchSize_Treat$CovrPrd2)
matchSize_Treat-matchSize_Control


#Modeled
matchSize_model1 <- lm(((CovrPrd2/2)-1) ~ SizeTreatLarge,data = matchSize_df, weights = weights)
matchSize_model2 <- clm(as.factor((CovrPrd2/2)-1) ~ SizeTreatLarge, data = matchSize_df, weights = weights)
...
#making a binary treatment is additionally challenging when the data have different categories of building (e.g. single and multi family), this is conditioned for in a regression setting with continuous value, but here we must conditionally dichotomize treatment ahead of time, otherwise matching on building types will be difficult.
                        ...
                        SFMedVal<- quantile(ProgramTarpTimeSF$VAPBLDG, 0.5)
                        MFMedVal<- quantile(ProgramTarpTimeMF$VAPBLDG, 0.5)
                        MoMedVal<- quantile(ProgramTarpTimeMo$VAPBLDG, 0.5)
                       OthMedVal<- quantile(ProgramTarpTimeOth$VAPBLDG, 0.5)


ProgramTarpTimeMatch2<- ProgramTarpTime%>%
                      dplyr::mutate(ValTreatLarge = ifelse(BuildingType == "SINGLE FAMILY" & VAPBLDG > as.numeric(SFMedVal), 1,
                                              ifelse(BuildingType == "MULTI-FAMILY" & VAPBLDG > as.numeric(MFMedVal), 1,
                                               ifelse(BuildingType == "MOBILE HOME" & VAPBLDG > as.numeric(MoMedVal), 1,
                                               ifelse(BuildingType == "Other" & VAPBLDG > as.numeric(OthMedVal), 1,
                                               0)))))
...
matchVal_Control<- mean(matchVal_Control$CovrPrd2)

matchVal_Treat<- matchVal_df%>%
                      filter(ValTreatLarge==1)
matchVal_Treat<- mean(matchVal_Treat$CovrPrd2)
matchVal_Treat-matchVal_Control


#Modeled
matchVal_model <- clm(as.factor((CovrPrd2/2)-1) ~ ValTreatLarge, data = matchVal_df, weights = weights)
...
```{r}
#Intercept Only
Reg1d <- clm(as.factor((CovrPrd2/2)-1)~ 1
           , data = NProgramTarpTime)

#Base Reg
Reg2d <- clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)
           +log(VAPBLDG)
           , data = NProgramTarpTime)
...
#Property Level Controls
Reg3d <- clm(as.factor((CovrPrd2/2)-1)~
           log(FootprintArea)
           +log(VAPBLDG)
           +log(X1mSWmphBnd)
           +age+I(age^2)
...
#Block Group socioeconomic controls
Reg4d <- clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)
           +log(VAPBLDG)
           +log(X1mSWmphBnd)
           +age+I(age^2)
...
# Zip Code FE
Reg5d <-  clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)
           +log(VAPBLDG)
           +log(X1mSWmphBnd)
           +age+I(age^2)
...
```{r}
#Intercept Only
Reg1e <- clm(as.factor((CovrPrd2/2)-1)~ 1
           , data = TarpTime)

#Base Reg
Reg2e <- clm(as.factor((CovrPrd2/2)-1)~
...
#Property Level Controls
Reg3e <- clm(as.factor((CovrPrd2/2)-1)~
           log(FootprintArea)*Program
           +log(VAPBLDG)*Program
...
#Block Group socioeconomic controls
Reg4e <- clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)*Program
           +log(VAPBLDG)*Program
...
# Zip Code FE
Reg5e <-  clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)*Program
           +log(VAPBLDG)*Program
...
```{r}
#Intercept Only
Reg1f <- clm(as.factor((CovrPrd2/2)-1)~ 1
           , data = filter(matchProgram_df, Program==1))

#Base Reg
Reg2f <- clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)
            +log(VAPBLDG)
            #+Program
           , data = filter(matchProgram_df, Program==1))
...
#Property Level Controls
Reg3f <- clm(as.factor((CovrPrd2/2)-1)~
           log(FootprintArea)
           +log(VAPBLDG)
           #+Program
           +log(X1mSWmphBnd)
...
#Block Group socioeconomic controls
Reg4f <- clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)
           +log(VAPBLDG)
           #+Program
           +log(X1mSWmphBnd)
...
# Zip Code FE
Reg5f <-  clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)
           +log(VAPBLDG)
           #+Program
           +log(X1mSWmphBnd)
...
```{r}
#Intercept Only
Reg1g <- clm(as.factor((CovrPrd2/2)-1)~ 1
           , data = matchProgram_df)

#Base Reg
Reg2g <- clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)*Program
           +VAPTOTAL*Program
           , data = matchProgram_df)
...
#Property Level Controls
Reg3g <- clm(as.factor((CovrPrd2/2)-1)~
           log(FootprintArea)*Program
           +log(VAPBLDG)*Program
           +log(X1mSWmphBnd)
           +age+I(age^2)
...
#Block Group socioeconomic controls
Reg4g <- clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)*Program
           +log(VAPBLDG)*Program
           +log(X1mSWmphBnd)
           +age+I(age^2)
...
# Zip Code FE
Reg5g <-  clm(as.factor((CovrPrd2/2)-1)~
            log(FootprintArea)*Program
           +log(VAPBLDG)*Program
           +log(X1mSWmphBnd)
           +age+I(age^2)
