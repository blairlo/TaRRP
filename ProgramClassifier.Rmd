...
```{r, fig.height = 6.5, fig.width = 20}
fishnet<- st_make_grid(ProgramPredictedEvaluated, cellsize = 2640, what = "polygons")%>%
    st_as_sf() %>%
   rename(geometry = x) %>%
...
PointCluster1<-st_centroid(ProgramPredictedEvaluated %>% filter(Type2Error == 1 |Type1Error == 1 ))

PointCluster1.ppp <- as.ppp(st_geometry(PointCluster1))
marks(PointCluster1.ppp)  <- NULL
PointCluster1.km <- rescale(PointCluster1.ppp, 1000, "km")

ann.p <- mean(nndist(PointCluster1.km, k=1))
ann.p

#Measure and Plot Counterfactual Density 
DensityRaster <- st_rasterize(fishnetStats %>% select(PrgrmPrdN, geometry))
DensityRaster.im <- as.im(DensityRaster)
DensityRaster.km <- rescale(DensityRaster.im, 1000, "km")

n     <- 5000L
ann.r <- vector(length=n)
for (i in 1:n){
  rand.p   <- rpoint(n=PointCluster1.km$n, f=DensityRaster.km) #simulate the same number of random points as the counter factual weighted by population density
