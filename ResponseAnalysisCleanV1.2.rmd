...
NNDist<-as.data.frame(NNDist)%>%
        mutate(NN = (dist.1+
                       ...
                       #dist.9+
                       #dist.10
                       /5))#Make Average distance of the first 5 neighbors
...
prTrimmed_df<-prs_df%>% filter(pr_score >= 0.2 & pr_score <= 0.8)
...
#make Poisson density * data for truncated distribution @ 5
xValue <- 0:4
...
Regout<-texreg(list(Program, NProgram_Match, Matched, Interact_Full_FE, Interact_Trimmed_FE, Interact_Matched_FE),
               #(custom.title = "Test, if rescaled, units in ()"),
               custom.model.names = c("(1) Program","(2) Non-Program","(3) Combined","(4) Combined","(5) Combined","(6) Combined"),
               ...
               custom.coef.names = c("Intercept",
                                     ...
                                     "Program Tarp : Perc. Non-White"
                                     ),
               reorder.coef = c(1,2,3,4,5,18,19,20,21,22,6,7,8,9,10,11,12,14,15,16,17,13),
               groups = list("Variables of Interest" = 2:10,
                             "Damage Related Controls" = 11:17,
                             "Access Related Controls" = 18:21),
               custom.gof.rows=(list("Zip Code FE" = c("Yes","Yes","Yes","Yes","Yes","Yes"),
                                     "Data" = c("Full","Matched","Matched","Full","Trimmed","Matched"))),
               #file = "Regout.html",
               )
...
mean4<-round(mean(matchProgram_df$nonwhiteP),2)
sd4<-round(sd(matchProgram_df$nonwhiteP),2)
...
Pred5<-ggeffect(Interact_Matched_FE_NS, terms = c(print(paste0("nonwhiteP","[", mean4,",",mean4+sd4,"]")),"Program"))
...
Pred5Matrix<-10*cbind(Pred5$predicted[3,]-Pred5$predicted[1,],
                   Pred5$predicted[4,]-Pred5$predicted[2,],
                  ((Pred5$predicted[4,]-Pred5$predicted[2,])-(Pred5$predicted[3,]-Pred5$predicted[1,])))
...
Pred5MatrixPerc<- 100*cbind((Pred5$predicted[3,]-Pred5$predicted[1,])/Pred5$predicted[1,],
                   (Pred5$predicted[4,]-Pred5$predicted[2,])/Pred5$predicted[2,], NA)

#make table (these are rearranged a bit to match results tables, hypothesis in the paper etc.)
...
Predx<-ggeffect(Interact_Matched_FE_NS, terms = c("FootprintArea","NNDist.NN[111,1386,4248]","Program"))
Pred5<-ggeffect(Interact_Matched_FE_NS, terms = c("nonwhiteP[all]","Program"))
...
p1<-plot(Pred1) +
         ...
         scale_y_continuous(labels = formatter10, limits = c(0.6,1.6))
...
p5<-plot(Predx)+
      ...
      scale_colour_manual(values = c("#F8766D", "#00BFC4","#619CFF"), 
                          labels = c( "Min (Very Dense)","Mean","+2sd(Less Dense)"))+
...
p6<-plot(Pred5)+
         labs(x = "Perc. Non-White", 
         y = "Estimated Days Elapsed", 
         title = "")+
         geom_rug(col="black",alpha=0.2, size=1)+
         scale_x_continuous(limits = c(0,mean4+sd4+sd4))+
         scale_y_continuous(labels = formatter10, limits = c(0.6,1.6))


#grid.arrange(arrangeGrob(p1, p2,p3,p4),p5, nrow = 3)
grid.arrange(p1,p3,p2,p6, nrow = 2)
p4
p5
